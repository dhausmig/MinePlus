...
-- the code below was written by dhausmig
	builder_to_tool = function(pn, pos, itemstack)
		local meta = minetest.get_meta(pos)
		local inv = meta:get_inventory()
		local target_item = inv:get_stack("main",1)
		local target_name = target_item:get_definition().name
		...
		if (digtron.config.builder_tool_verbose_read) then
			minetest.chat_send_player(pn, "The digtron:builder at "..pos.x..","..pos.y..
			","..pos.z.." builds "..target_name.." with extrusion="..extrusion..
			", period="..period..", offset="..offset..", build_facing="..build_facing)
		end
		local tmeta = itemstack:get_meta()
		tmeta:set_string("target_name", target_name)
		tmeta:set_int("extrusion", extrusion)
		tmeta:set_int("period", period)
		tmeta:set_int("offset", offset)
		tmeta:set_int("build_facing", build_facing)
		return itemstack
	end
--=============================================
	tool_to_builder = function(pn, itemstack, pos)
		local tmeta = itemstack:get_meta()
		local target_name  = tmeta:get_string("target_name")
		local extrusion = tmeta:get_int("extrusion")
		local period = tmeta:get_int("period")
		local offset = tmeta:get_int("offset")
		local build_facing = tmeta:get_int("build_facing")
		if (digtron.config.builder_tool_verbose_write) then
			minetest.chat_send_player(pn, "Setting digtron:builder at "..pos.x..","..pos.y..
			","..pos.z.." to "..target_name.." with extrusion="..extrusion..
			", period="..period..", offset="..offset..", build_facing="..build_facing)
		end
		local meta = minetest.get_meta(pos)
		local inv = meta:get_inventory()
		inv:set_stack("main", 1, target_name)
		meta:set_int("extrusion", extrusion)
		meta:set_int("period", period)
		meta:set_int("offset", offset)
		meta:set_int("build_facing", build_facing)
		digtron.update_builder_item(pos)
		return nil
	end
--=============================================
minetest.register_tool("digtron:builder_tool", {
	type = "none",
	description = "Digtron Builder Tool",
	inventory_image = "digtron_builder_tool.png",
	range = 6.0,
--=============================================
	on_place = function(itemstack, placer, pointed_thing)
		...
		if (pos) then
			minetest.chat_send_player(pn, "Position = "..pos.x..", "..pos.y..", "..pos.z)
		else
			minetest.chat_send_player(pn, "Not pointing at anything")
			return nil
		end
		...
		return builder_to_tool(pn, pos, itemstack)
	end,
	--=============================================
	on_use = function(itemstack, user, pointed_thing)
		...
		local node = minetest.get_node(pos)
		minetest.chat_send_player(pn, "Pointing at ".. pointed_thing.type)
		minetest.chat_send_player(pn, node.name)
		if (node.name ~= "digtron:builder") then
			return nil
		end
		return tool_to_builder(pn, itemstack, pos)
	end,
	--============================================
	tool_capabilities = {
...
minetest.register_craft({
	...
	recipe = {
		...
		{'default:mese', 'digtron:builder', 'default:mese'},
		{'default:diamondblock', 'default:mese', 'default:diamondblock'},
	}
})
