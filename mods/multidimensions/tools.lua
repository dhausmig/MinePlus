...
--			player:set_physics_override({gravity=v.gravity})
			playerphysics.add_physics_factor(player, "gravity", "dim_grav", v.gravity)
			if v.sky then
				player:set_sky(v.sky)
			else
				player:set_sky({base_color = ff8c8afa, type = "regular"})
			end
			if v.on_enter then
				v.on_enter(player)
			end
...
--	player:set_physics_override({gravity=1})
	playerphysics.remove_physics_factor(player, "gravity", "dim_grav")
