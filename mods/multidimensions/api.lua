...
multidimensions.register_dimension=function(name,def,self)
	...
	if def.teleporter then
		...
		node.tiles = node.tiles or			{"default_transport.png"}
		...
		if multidimensions.craftable_teleporters and craft then
			...
			minetest.register_craft({
				type = "shapeless",
				output = "multidimensions:dimension_disc",
				recipe = {"multidimensions:teleporter_" .. name},
			})
		end
	end
...
minetest.register_node("multidimensions:blocking", {
	...
	drawtype="airlike",
	groups = {unbreakable=1,not_in_creative_inventory = 1,fall_damage_add_percent= -0.50},
	paramtype = "light",
	sunlight_propagates = true,
	drop = "",
	pointable=false,
