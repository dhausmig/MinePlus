...
local S = minetest.get_translator("decor_wardrobe")
modpath = minetest.get_modpath("decor_wardrobe")
...
if skinsdb_mod_path then
	...
	for _, shrt in ipairs(skinslist) do
		...
		for _, prefix in ipairs({"", "fe"}) do
			...
			local skin_obj = skins.new("decor_clothes_"..skin_name..".png")

			skin_obj:set_preview("decor_clothes_"..skin_name.."_preview.png")
			skin_obj:set_texture("decor_clothes_"..skin_name..".png")
			...
			local file = io.open(modpath ..
				"/textures/decor_clothes_" .. skin_name .. ".png", "r")
...
local function set_player_skin(player, skin, save)
	...
	if save and not skinsdb_mod_path then
		...
		if skin == default_skin then
			player:get_meta():set_string("decor:player_skin", "")
		else
			player:get_meta():set_string("decor:player_skin", skin)
		end
	end
end
...
local def = {
	...
	drawtype = "mesh",
	mesh = "decor_bedroom_wardrobe.obj",
	tiles = {
		{name = "decor_generic_wood_plain.png", color = 0xffa76820},
		"decor_wardrobe_drawers.png",
		"decor_wardrobe_doors.png"
	},
	inventory_image = "decor_wardrobe_inv.png",
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		return decor.stack_vertically(itemstack, placer, pointed_thing,
	...
	on_construct = function(pos)
		...
		for i = 1, 5 do
			...
			clothes_strings = clothes_strings ..
				...
				".5,0;1.1,2;decor_clothes_" .. skinslist[i] ..
				"_preview.png;" .. skinslist[i] .. ";]" ..
				"image_button_exit[" .. (i-1) ..
				".5,2;1.1,2;decor_clothes_fe" .. skinslist[i] ..
		...
		for i = 1, 5 do
			...
			if fields[skinslist[i]] then
				...
				set_player_skin(sender,
					"decor_clothes_" .. skinslist[i] .. ".png", "player")
				break

			elseif fields["fe" .. skinslist[i]] then

				set_player_skin(sender,
					"decor_clothes_fe" .. skinslist[i] .. ".png", "player")
				break
			end
		end
	end
...
minetest.register_node(":decor:wardrobe", def)

minetest.register_alias("decor:wardrobe_bottom", "decor:wardrobe")
minetest.register_alias("decor:wardrobe_top", "air")

if not skinsdb_mod_path then -- If not managed by skinsdb

	minetest.register_on_joinplayer(function(player)

		local skin = player:get_meta():get("decor:player_skin")
...
minetest.register_craft( {
	output = "decor:wardrobe",
	recipe = {
		{ "decor:drawer_small", "decor:kitchen_cabinet" },
