...
minetest.register_alias("decor:expansion_placeholder", "air")
...
function decor.stack_vertically(itemstack, placer, pointed_thing, node1, node2)
...
-- like  decor.stack_vertically but tests first if it was placed as a right wing, then uses node1_right and node2_right instead

function decor.stack_wing(itemstack, placer, pointed_thing, node1, node2, node1_right, node2_right)
	local rightclick_result = rightclick_pointed_thing(pointed_thing.under, placer, itemstack, pointed_thing)
	if rightclick_result then return rightclick_result end
	...
	local is_right_wing = node1 == minetest.get_node(
		{
			x = pos.x + decor.fdir_to_left[fdir+1][1],
			y = pos.y,
			z = pos.z + decor.fdir_to_left[fdir+1][2] }).name
...
function decor.stack_sideways(itemstack, placer, pointed_thing, node1, node2, dir)
	local rightclick_result = rightclick_pointed_thing(pointed_thing.under, placer, itemstack, pointed_thing)
	if rightclick_result then return rightclick_result end
	...
	local fdir = minetest.dir_to_facedir(placer:get_look_dir())
	local fdir_transform = dir and decor.fdir_to_right or decor.fdir_to_fwd
...
function decor.bed_expansion(pos, placer, itemstack, pointed_thing, trybunks)

	local thisnode = minetest.get_node(pos)
	local param2 = thisnode.param2
	local fdir = param2 % 8

	local fxd = decor.wall_fdir_to_fwd[fdir+1][1]
	local fzd = decor.wall_fdir_to_fwd[fdir+1][2]
...
function decor.place_banister(itemstack, placer, pointed_thing)
	local rightclick_result = rightclick_pointed_thing(pointed_thing.under, placer, itemstack, pointed_thing)
	if rightclick_result then return rightclick_result end
	...
	local lxd = decor.fdir_to_left[fdir+1][1]
	local lzd = decor.fdir_to_left[fdir+1][2]

	local rxd = decor.fdir_to_right[fdir+1][1]
	local rzd = decor.fdir_to_right[fdir+1][2]

	local fxd = decor.fdir_to_fwd[fdir+1][1]
	local fzd = decor.fdir_to_fwd[fdir+1][2]
	...
	elseif left_below_node and string.find(left_below_node.name, "decor:banister_.*_diagonal") then
		fdir = left_below_node.param2
		new_place_name = string.gsub(left_below_node.name, "_diagonal_.-$", "_horizontal")
	elseif right_below_node and string.find(right_below_node.name, "decor:banister_.*_diagonal") then
	...
	elseif left_fwd_below_node and string.find(left_fwd_below_node.name, "decor:banister_.*_diagonal")
	...
	elseif right_fwd_below_node and string.find(right_fwd_below_node.name, "decor:banister_.*_diagonal")
	  and is_buildable_to(placer_name, fwd_pos, fwd_pos) then
	...
	elseif left_node and string.find(left_node.name, "decor:banister_.*_diagonal") then
		fdir = left_node.param2
		new_place_name = string.gsub(left_node.name, "_diagonal_.-$", "_horizontal")
	elseif right_node and string.find(right_node.name, "decor:banister_.*_diagonal") then
	...
	elseif left_fwd_node and string.find(left_fwd_node.name, "decor:banister_.*_diagonal")
	  and is_buildable_to(placer_name, fwd_pos, fwd_pos) then
		fdir = left_fwd_node.param2
		pos = fwd_pos
		new_place_name = string.gsub(left_fwd_node.name, "_diagonal_.-$", "_horizontal")
	elseif right_fwd_node and string.find(right_fwd_node.name, "decor:banister_.*_diagonal")
	  and is_buildable_to(placer_name, fwd_pos, fwd_pos) then
