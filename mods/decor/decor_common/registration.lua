...
function decor.register(name, original_def)
	...
	decor.handle_inventory(name, def, original_def)
	...
	if expand then
		...
		def.on_place = def.on_place or function(itemstack, placer, pointed_thing)
			if expand.top then
				return decor.stack_vertically(itemstack, placer, pointed_thing, itemstack:get_name(), expand.top)
			elseif expand.right then
				return decor.stack_sideways(itemstack, placer, pointed_thing, itemstack:get_name(), expand.right, true)
			elseif expand.forward then
				return decor.stack_sideways(itemstack, placer, pointed_thing, itemstack:get_name(), expand.forward, false)
			end
		end
		def.after_dig_node = def.after_dig_node or function(pos, oldnode, oldmetadata, digger)
			...
			if expand.right and expand.forward ~= "air" then
				local right_pos = {
					x = pos.x + decor.fdir_to_right[fdir+1][1],
					y = pos.y,
					z = pos.z + decor.fdir_to_right[fdir+1][2]
				}
			...
			if expand.forward and expand.forward ~= "air" then
				local forward_pos = { x=pos.x+decor.fdir_to_fwd[fdir+1][1], y=pos.y, z=pos.z+decor.fdir_to_fwd[fdir+1][2] }
