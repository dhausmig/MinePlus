...
-- #### lines 439-441 commented out due to error
-- Features:
...
-- doors
basic_houses.door_bottom = "doors:door_wood_win_a";
...
basic_houses.simple_hut_place_hut = function( data, materials, heightmap )
	local p = data.p2;
	if (p.x <150 and p.x > -150) then -- avoid putting houses in the lower road
		return nil;
	end
	local sizex = data.sizex-1;
	local sizez = data.sizez-1;
	-- house too small or too large
	if( sizex < 8 or sizez < 8 or sizex>64 or sizez>64) then
		return nil;
	end
...
-- TODO: avoid overlapping with mg_villages if that one is installed
	local sizex = math.random(11,maxsize);
	local sizez = math.random(11,maxsize);
--	local sizez = math.max( 8, math.min( maxsize, math.random( math.floor(sizex/4), sizex*2 )));
	...
	local minheight = 2 -- one above water level; avoid below water level and places on ice
	local maxheight = 1000 -- keep them on the ground
	return basic_houses.simple_hut_find_place( heightmap, minp, maxp, sizex, sizez, minheight, maxheight );
end
...
-- used for debugging
local show_woods = false
if show_woods then
	local wood_types = replacements_group['wood'].found;
	for i = 1, #wood_types do
		minetest.log(wood_types[i] )
	end
end
