...
-- its own issues (e.g. nodeId_top not getting set correctly), so I'm leaving overdraw off (i.e. zero) until I
...
cloudlands.coreTypes = {
  ...
  },
  {
    territorySize     = 34, --30,
    coresPerTerritory = 3,
    radiusMax         = 18, --16, -- I feel this and depthMax should be bigger, say 18, and territorySize increased to 34 to match, but I can't change it any more or existing worlds will mismatch along previously emerged chunk boundaries
    depthMax          = 20, --16,
    thicknessMax      = 2,
    frequency         = 50.0, --0.1,
...
--[[
-- for debugging
minetest.log("cloudlands altitude = "..ALTITUDE)
minetest.log("cloudlands amplitude = "..ALTITUDE_AMPLITUDE)
minetest.log("cloudlands lower = "..ALTITUDE - ALTITUDE_AMPLITUDE)
minetest.log("cloudlands upper = "..ALTITUDE + ALTITUDE_AMPLITUDE)
--]]
...
-- optional region specified in settings to restrict islands to
...
local function init_secrets()
  ...
  nodeId_junk       = interop.find_node_id({"xdecor:barrel", "cottages:barrel", "decor:copper_pans", "vessels:steel_bottle", "mcl_flowerpots:flower_pot"})
  ...
  nodeId_workbench  = interop.find_node_id({"decor:table", "xdecor:workbench", "mcl_crafting_table:crafting_table", "default:table", "random_buildings:bench", "craftingtable:craftingtable"}) -- "default:table" isn't a thing, but perhaps one day.
  nodeId_cobweb     = interop.find_node_id({"mobs:cobweb", "xdecor:cobweb", "decor:cobweb_plantlike", "default:cobweb", "main:cobweb"})
...
local function renderCores(cores, minp, maxp, blockseed)
  ...
  if voxelsWereManipulated then
    ...
    --    minetest.set_mapgen_ params({mgname = "singlenode", flags = "nolight"})
    ...
    -- values to zero at islands before calling calc_lighting() to propagate lighting
    -- down from above.
    --
...
local function on_generated(minp, maxp, blockseed)
  ...
  if minp.y > ALTITUDE + (ALTITUDE_AMPLITUDE + maxSufaceRise + 10) or
  -- the 10 is an arbitrary number because sometimes the noise values exceed their normal range.
...
minetest.register_on_mapgen_init(
  ...
  function(mgparams)
    ...
    --if DEBUG then minetest.set_map gen_params({mgname = "singlenode"--[[, flags = "nolight"]]}) end
  end
)
