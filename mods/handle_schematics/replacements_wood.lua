...
replacements_group['wood'].add_material = function( candidate_list, mod_prefix, w_pre, w_post, t_pre, t_post, l_pre, l_post,
	...
	for _,v in ipairs( candidate_list ) do
		...
		elseif( mod_prefix=='trees:' and w_post=='_wood' and t_post=='_log' ) then
			...
			data[20] = 'trees:'..v..'_wood_stair_upside_down';
			data[21] = 'trees:'..v..'_wood_slab_upside_down';
...
replacements_group['wood'].construct_wood_type_list = function()
	...
	replacements_group['wood'].add_material( moretrees_treelist,  'moretrees:', '', '_wood', '','_trunk', '','_leaves','','_sapling',
		mt_pre_stair,'_wood', mt_pre_slab,'_wood', 'NONE','',  'NONE','');

--	candidate_list, mod_prefix,
--	w_pre, w_post,
--	t_pre, t_post,
--	l_pre, l_post,
--	s_pre, s_post,
--	stair_pre, stair_post,
--	slab_pre, slab_post,
--	fence_pre, fence_post,
--	gate_pre, gate_post 	

	local plustrees_treelist = {"baldcypress", "banana_tree", "cherrytree", "chestnuttree", "clementinetree", "ebony",
		"hollytree", "jacaranda","larch", "lemontree", "mahogany", "maple", "metasequoia", "orange_tree", "plumtree", "pomegranate", 
		"redoak", "willow" }  -- pulled "bamboo",  due to the trunk not filling full nodes
	for i = 1 , #plustrees_treelist do
		replacements_group['wood'].add_material( {''}, plustrees_treelist[i]..":",
		"", 'wood',
		"", 'trunk',
		"", 'leaves',
		"", 'sapling',
		'stairs:stair_', plustrees_treelist[i],
		'stairs:slab_', plustrees_treelist[i],
		'', 'fence','',
		'doors:gate_', plustrees_treelist[i]..'_closed' );
	end
...
-- pulled "black_wattle", "bull_banksia", "cherry", "golden_wattle", "grey_mangrove",
--			"lemon_myrtle", "lilly_pilly", "macadamia", "mangrove_apple", "mulga",
--			"quandong", "red_bottlebrush", "rottnest_island_pine", "stilted_mangrove",
--			"swamp_paperbark", "tea_tree", "wirewood", because trunk diameter too small

		replacements_group['wood'].add_material( aus_treelist, "australia:",
		"", '_wood',
		"", '_trunk',
		"", '_leaves',
		"", '_sapling',
