...
local function tnt_explode(pos, radius, ignore_protection, ignore_on_blast, owner, explode_center)
	...
	if minetest.settings:get_bool("explosions_underground") and pos.y > -100 then return end  -- keep explosions underground
...
function tnt.register_tnt(def)
	...
	if enable_tnt then
		minetest.register_node(":" .. name, {
			...
			on_punch = function(pos, node, puncher)
				if puncher:get_wielded_item():get_name() == "default:torch" then
				   if (pos.y < -79) then		-- limit usage to subterranean
					   minetest.swap_node(pos, {name = name .. "_burning"})
