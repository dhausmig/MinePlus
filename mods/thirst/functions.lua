...
Core functions for Thirst.

See init.lua for license.

]]

local PPA = thirst.persistent_player_attributes
...
function thirst.on_joinplayer(player)
    local name = player:get_player_name()
    -- default entry for new players
    if not thirst.players[name] then
    ...
    thirst.hud_init(player)
end

function thirst.on_dieplayer(player)
    local name = player:get_player_name()
    local pl   = thirst.players[name]
    -- reset after death
    PPA.set_value(player, 'thirst_hydro', 20)
    pl.pending_dmg = 0.0
    pl.thirst_factor = 0.5
end
...
function thirst.drink(player, value, max)
    -- if max is not specified, assume 20
    if not max then
        max = 20
    end
    local hydro = PPA.get_value(player, 'thirst_hydro')
    ...
    if hydro < max then
        ...
        PPA.set_value(player, 'thirst_hydro', hydro)
        return true
    end
    return false
end

function thirst.get_hydro(player)
    return PPA.get_value(player, 'thirst_hydro')
end

function thirst.set_thirst_factor(player, factor)
    local name = player:get_player_name()
    local pl = thirst.players[name]
    pl.thirst_factor = factor
end

function thirst.get_thirst_factor(player)
    local name = player:get_player_name()
    local pl = thirst.players[name]
...
function thirst.main_loop(dtime)
    -- get thirst
    thirst.time_next_tick = thirst.time_next_tick - dtime
    while thirst.time_next_tick < 0.0 do
        -- time for thirst
        thirst.time_next_tick = thirst.time_next_tick + thirst.config.tick_time
        for _,player in ipairs(minetest.get_connected_players()) do
            ...
            if pl.last_pos == pos_hash then
                pl.time_in_pos = pl.time_in_pos + thirst.config.tick_time
            else
                -- you moved!
                pl.last_pos = pos_hash
                pl.time_in_pos = 0.0
            end
            local pl_standing = pl.time_in_pos > thirst.config.stand_still_for_drink
            local pl_afk      = pl.time_in_pos > thirst.config.stand_still_for_afk
            --print("Standing: " .. (pl_standing and 'true' or 'false' ) .. ", AFK: " .. (pl_afk and 'true' or 'false'))

            pos.y = pos.y + 0.1
            local node = minetest.get_node(pos)
            local drink_per_second = thirst.config.regen_from_node[node.name] or 0

            -- fountaining (uses pos, slight changes ok)
            for k, fountain in pairs(thirst.fountains) do
                local dx = fountain.pos.x - pos.x
                local dy = fountain.pos.y - pos.y
                local dz = fountain.pos.z - pos.z
                local dist2 = dx * dx + dy * dy + dz * dz
                local fdist = fountain.level * thirst.config.fountain_distance_per_level -- max 100 nodes radius
                ...
                if dist2 < fdist * fdist then
                    ...
                    drink_per_second = math.max(thirst.config.regen_from_fountain or 0, drink_per_second)
                    pl_standing = true
            ...
            for i, itemstack in ipairs(inv_main) do
                ...
                local injector_this = thirst.config.injection_for_item[name]
                if injector_this and injector_this > injector_max then
                    injector_max = injector_this
                end
                local extractor_this = thirst.config.extraction_for_item[name]
                if extractor_this and extractor_this > extractor_max then
                    extractor_max = extractor_this
                end
                if thirst.config.container_capacity[name] then
            ...
            if extractor_max > 0.0 and container_not_full then
                ...
                local capacity = thirst.config.container_capacity[itemstack:get_name()]
                local wear = itemstack:get_wear()
                if wear == 0 then wear = 65535.0 end
                local drink = extractor_max * thirst.config.tick_time
            ...
            if injector_max > 0.0 and container_not_empty then
                ...
                local wear = itemstack:get_wear()
                if wear == 0 then wear = 65535.0 end
                local drink = injector_max * thirst.config.tick_time
                ...
                itemstack:set_wear(wear)
                thirst.drink(player, drink, 20)
                hydro = PPA.get_value(player, 'thirst_hydro')
                player:get_inventory():set_stack("main", i, itemstack)
            end
            ...
            if drink_per_second > 0 and pl_standing then
                ...
                thirst.drink(player, drink_per_second * thirst.config.tick_time, 20)
                --print("Raising hydration by "..(drink_per_second*thirst.config.tick_time).." to "..PPA.get_value(player, 'thirst_hydro'))
            else
                if not pl_afk then
                    -- only get thirsty if not AFK
                    local amount = thirst.config.thirst_per_second * thirst.config.tick_time * pl.thirst_factor
                    PPA.set_value(player, 'thirst_hydro', hydro - amount)
                    hydro = PPA.get_value(player, 'thirst_hydro')
            ...
            thirst.hud_update(player, hydro)
            ...
            if minetest.settings:get_bool("enable_damage") then
                ...
                if hydro <= 0.0 and not pl_afk then
                    pl.pending_dmg = pl.pending_dmg + thirst.config.damage_per_second * thirst.config.tick_time
...
function thirst.drink_handler(player, itemstack, node)
    local pl = thirst.players[player:get_player_name()]
    local hydro = PPA.get_value(player, 'thirst_hydro')
    ...
    if thirst.config.node_drinkable[node_name] then
        -- we found something to drink!
        local cont_level = thirst.config.drink_from_container[item_name] or 0
        local node_level = thirst.config.drink_from_node[node_name] or 0
        -- drink until level
        local level = math.max(cont_level, node_level)
        --print("Drinking to level " .. level)
        thirst.drink(player, level, level)

        -- fill container, if applicable
        if thirst.config.container_capacity[item_name] then
            --print("Filling a " .. item_name .. " to " .. thirst.config.container_capacity[item_name])
            itemstack:set_wear(1) -- "looks full"
        end

    elseif thirst.config.container_capacity[item_name] then
        -- drinking from a container
        if itemstack:get_wear() ~= 0 then
            local capacity = thirst.config.container_capacity[item_name]
            ...
            if hydro_missing > 0 then
                ...
                if wear_missing > 0 then -- rounding glitches?
                    thirst.drink(player, wear_missing * capacity / 65535.0, 20)
                    hydro = PPA.get_value(player, 'thirst_hydro')
                end
            end
        end
...
function thirst.on_use( old_on_use )
    return function(itemstack, player, pointed_thing)
        ...
        thirst.drink_handler(player, itemstack, node)
...
function thirst.on_rightclick( old_on_rightclick )
...
function thirst.augment_item_for_drinking( itemname, level )
    ...
    new_definition.on_use = thirst.on_use(
    ...
    thirst.config.drink_from_container[itemname] = level
end

function thirst.fountain_abm(pos, node)
    local fountain_count = 0
    local water_count = 0
    local total_count = 0
    for y = 0, thirst.config.fountain_height do
        for x = -y, y do
            for z = -y, y do
                ...
                if n then
                    ...
                    local type = thirst.config.fountain_type[n.name] or ''
                    if type == 'f' then
    ...
    local level = math.min(thirst.config.fountain_max_level, math.min(fountain_count, water_count))
    --print(string.format("Fountain (%d): %d + %d / %d", level, fountain_count, water_count, total_count))
    thirst.fountains[string.format("%d:%d:%d", pos.x, pos.y, pos.z)] = {
