...
A Mineplus mod that adds a "thirst" mechanic.
...
This is a mod for MinePlus. It adds a thirst mechanic to the
...
* thirst.drink(player, amount, [max]) : instantly drink a bit (up to a max value, default 20)
* thirst.get_hydro(player) : returns the current hydration of a player
* thirst.set_thirst_factor(player, factor) : how fast does the given player get thirsty (default is 1.0)
* thirst.get_thirst_factor(player) : returns the current thirst factor of a player
...
Unzip the archive, rename the folder to `thirst` and
