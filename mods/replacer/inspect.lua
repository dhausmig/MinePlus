...
replacer.add_table_saw_receipe = function( node_name, receipes )
	if( not( node_name ) or not( minetest.get_modpath("moreblocks")) or not( table_saw ) or not( table_saw.names) or (node_name=='moreblocks:table_saw')) then
		return;
	end
...
--	for i,v in ipairs( table_saw.names ) do
...
replacer.inspect_show_crafting = function( name, node_name, fields )
	...
	replacer.add_table_saw_receipe( node_name, res );
	...
	if( not( res ) or #res<1) then
		...
		elseif( receipe.type=='saw'          and receipe.items and #receipe.items==1 ) then
			--formspec = formspec.."item_image[1,1;3.4,3.4;moreblocks:table_saw]"..
			formspec = formspec.."item_image_button[1,1;3.4,3.4;"..replacer.image_button_link( 'moreblocks:table_saw' ).."]"..
