...
local function tunneler_config( tunneler, player, point )
	...
	--show the formspec
	minetest.show_formspec( player:get_player_name(), "tunneler:tunneler_config", formspec )
	return( tunneler)
end
...
local function tunneler_field_handler( player, formname, fields )
	...
	if formname ~= "tunneler:tunneler_config" then
		return false
	end
...
local function tunneler_on_dig( pos, node, player )
	...
	if string.find(player:get_wielded_item():get_name(), "tunneler:.*_tunneler") == nil then
		return
	end
