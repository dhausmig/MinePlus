...
-- [function] Colorize
function chat3.colorize(name, color, msg)
		...
		local vers = prot[rname]
			...
			if chat3.is_mentioned(rname, msg) then
				...
				if bell and name ~= rname then
					local pbell = player:get_meta():get("chat3:bell")
					if pbell ~= "false" then
			...
			-- Send message
			local send = chat3.colorize(rname, color, "<"..name.."> "..msg)
			if prefix then
				send = prefix..send
			end
...
if bell then
	minetest.register_chatcommand("chatbell", {
		...
		func = function(name)
			...
			if player then
				local bell = player:get_meta():get("chat3:bell")
				if not bell or bell == "" or bell == "true" then
					player:get_meta():set_string("chat3:bell", "false")
					return true, "Disabled Chatbell"
				else
					player:get_meta():set_string("chat3:bell", "true")
