...
table_saw.known_stairs = setmetatable({}, {
	__newindex = function(k, v)
		local modname = minetest.get_current_modname()
		print(("WARNING: mod %s tried to add node %s to the table saw"
				.. " manually."):format(modname, v))
	end,
})

-- This is populated by stairsplus:register_all:
table_saw.known_nodes = {}

-- How many microblocks does this shape at the output inventory cost:
-- It may cause slight loss, but no gain.
table_saw.cost_in_microblocks = {
	1, 1, 1, 1, 1, 1, 1, 2,
	2, 3, 2, 4, 2, 4, 5, 6,
	7, 0.5, 1, 2, 4, 6, 7, 8,
	1, 2, 2, 3, 1, 1, 2, 4,
...
function table_saw:get_cost(inv, stackname)
	for i, item in pairs(inv:get_list("output")) do
		if item:get_name() == stackname then
			return table_saw.cost_in_microblocks[i]
		end
	end
end

function table_saw:get_output_inv(modname, material, amount, max)
	...
	for i = 1, #table_saw.names do
		local t = table_saw.names[i]
		local cost = table_saw.cost_in_microblocks[i]
...
-- Reset empty table_saw after last full block has been taken out
-- (or the table_saw has been placed the first time)
-- Note: max_offered is not reset:
function table_saw:reset(pos)
...
function table_saw:update_inventory(pos, amount)
	...
	local name_parts = table_saw.known_nodes[node_name] or ""
	...
	-- print("table_saw set to " .. modname .. " : "
...
function table_saw.on_receive_fields(pos, formname, fields, sender)
	local meta = minetest.get_meta(pos)
	local max = tonumber(fields.max_offered)
	if max and max > 0 then
		meta:set_string("max_offered",  max)
		-- Update to show the correct number of items:
		table_saw:update_inventory(pos, 0)
	end
end


-- Moving the inventory of the table_saw around is not allowed because it
-- is a fictional inventory. Moving inventory around would be rather
-- impractical and make things more difficult to calculate:
function table_saw.allow_metadata_inventory_move(
...
function table_saw.allow_metadata_inventory_put(
	...
	if listname == "recycle" then
		...
		local cost = table_saw:get_cost(inv, stackname)
	...
	if listname == "input" then
		...
		for name, t in pairs(table_saw.known_nodes) do
...
function table_saw.on_metadata_inventory_put(
		pos, listname, index, stack, player)
	-- We need to find out if the table_saw is already set to a
	...
	if listname == "input" then
		...
		table_saw:update_inventory(pos, 8 * count)
	elseif listname == "recycle" then
		...
		if input_stack:get_count() + ((cost * count) / 8) <= input_stack:get_stack_max() then
			table_saw:update_inventory(pos, cost * count)
		end
	end
end

function table_saw.allow_metadata_inventory_take(pos, listname, index, stack, player)
	local meta          = minetest.get_meta(pos)
	local inv           = meta:get_inventory()
...
function table_saw.on_metadata_inventory_take(
		pos, listname, index, stack, player)
	...
	if listname == "output" then
		...
		local cost = table_saw.cost_in_microblocks[index]
				* stack:get_count()

		table_saw:update_inventory(pos, -cost)
	elseif listname == "micro" then
		-- Each microblock costs 1 microblock:
		table_saw:update_inventory(pos, -stack:get_count())
	elseif listname == "input" then
		-- Each normal (= full) block taken costs 8 microblocks:
		table_saw:update_inventory(pos, 8 * -stack:get_count())
	end
	-- The recycle field plays no role here since it is processed immediately.
end

function table_saw.on_construct(pos)
	local meta = minetest.get_meta(pos)
...
function table_saw.can_dig(pos,player)
	local meta = minetest.get_meta(pos)
	local inv = meta:get_inventory()
...
minetest.register_node("moreblocks:table_saw",  {
	...
	tiles = {"moreblocks_table_saw_top.png",
		"moreblocks_table_saw_bottom.png",
		"moreblocks_table_saw_side.png"},
	paramtype = "light",
	sunlight_propagates = true,
	paramtype2 = "facedir",
	groups = {choppy = 2,oddly_breakable_by_hand = 2},
	sounds = default.node_sound_wood_defaults(),
	on_construct = table_saw.on_construct,
	can_dig = table_saw.can_dig,
	-- Set the owner of this table saw.
	...
	-- The amount of items offered per shape can be configured:
	on_receive_fields = table_saw.on_receive_fields,
	allow_metadata_inventory_move = table_saw.allow_metadata_inventory_move,
	-- Only input- and recycle-slot are intended as input slots:
	allow_metadata_inventory_put = table_saw.allow_metadata_inventory_put,
	allow_metadata_inventory_take = table_saw.allow_metadata_inventory_take,
	-- Taking is allowed from all slots (even the internal microblock slot). Moving is forbidden.
	-- Putting something in is slightly more complicated than taking anything because we have to make sure it is of a suitable material:
	on_metadata_inventory_put = table_saw.on_metadata_inventory_put,
	on_metadata_inventory_take = table_saw.on_metadata_inventory_take,
})
