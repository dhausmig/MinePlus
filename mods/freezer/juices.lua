...
for juice_name, def in pairs(juice_table) do
	if minetest.get_modpath(def.found_in) then
		...
		-- higher nutritional value plus compensated by leaving a fancy stick behind
		minetest.register_craftitem("freezer:" .. juice_name .. "_popsicle", {
			...
			on_use = minetest.item_eat(def.orig_nutritional_value+2, "default:stick"),
		})
