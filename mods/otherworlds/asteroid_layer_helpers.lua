...
local ORECHA = 16 -- more than 1/27 3*3*3 -- Ore 1/x chance per stone node.
...
function otherworlds.asteroids.create_on_generated(ymin, ymax, content_ids)
	...
	local c_titaniumore = content_ids.c_titaniumore
						...
						if noise1dep >= STOT or noise4dep >= STOT then
							...
							if math.random(ORECHA) == 2 then
								...
								elseif nvals5[ni] < -0.4 then
									data[vi] = c_titaniumore
								elseif nvals5[ni] > 0.2 then
									data[vi] = c_meseore
								elseif nvals5[ni] < -0.1 then
									data[vi] = c_copperore
								else
									data[vi] = c_ironore
								end
