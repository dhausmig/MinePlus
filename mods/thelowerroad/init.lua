...
-- default: 30
local sinspread = tonumber(minetest.settings:get("thelowerroad.sinspread")) or 30
...
-- default: 5
local house_rareness = tonumber(minetest.settings:get("thelowerroad.house_rareness")) or 0
...
local excessive_clearing = tonumber(minetest.settings:get("thelowerroad.excessive_clearing")) or 0
-- road base elevation
-- in blocks above sea-level
local elevation_offset = tonumber(minetest.settings:get("thelowerroad.roadelevation")) or 0
...
local rbtorch = { name = "default:mese_post_light", param2 = 1, force_place = true, prob = 49 }
local rbtorch2 = { name = "default:mese_post_light", param2 = 1, force_place = false, prob = 255 }
local rbtorch3 = { name = "default:mese_post_light", param2 = 1, force_place = true, prob = 255 }
...
local chunksizeinnodes = minetest.settings:get("chunksize") * 16
...
local road_base_elevation = minetest.settings:get("water_level") + elevation_offset
local waterlevel = minetest.settings:get("water_level")
...
minetest.register_on_generated(function(minp, maxp, seed)
	...
	if (hashouse == 1) then
		...
		-- put in some loot
		local stack = ItemStack("food:bread " .. math.random(1, 11))
		chestinv:add_item("main", stack)
		stack = ItemStack("default:mese_post_light " .. math.random(1, 7))
		chestinv:add_item("main", stack)
