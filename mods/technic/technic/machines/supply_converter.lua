...
local run = function(pos, node, run_stage)
	...
	if from and to then
		...
		meta:set_string("infotext", S(""..machine_name..technic.pretty_num(input)..from..technic.pretty_num(input * remain)..to))
	else
		meta:set_string("infotext", S(""..machine_name.." Has Bad Cabling"))
		if to then
