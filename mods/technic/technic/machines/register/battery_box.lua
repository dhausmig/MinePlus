...
minetest.register_craft({
	...
	recipe = {
		{"group:wood",         "technic:sulfur_lump",   "group:wood"},
		{"technic:lead_ingot", "default:oil_extract", "technic:lead_ingot"},
		{"group:wood",         "technic:sulfur_lump",   "group:wood"},
	}
})
-- With sulfur dust:
...
function technic.register_battery_box(data)
	...
	local run = function(pos, node)
		...
		if not technic.is_tier_cable(below.name, tier) then
			meta:set_string("infotext", S(""..tier.." Battery Box Has No Network"))
			return
		end
		...
		local infotext = S(""..tier.." Battery Box: "..technic.pretty_num(current_charge)..technic.pretty_num(max_charge))
		if eu_input == 0 then
			infotext = S(""..infotext.." Idle")
		end
	...
	for i = 0, 8 do
		...
		minetest.register_node("technic:"..ltier.."_battery_box"..i, {
			description = S(""..tier.." Battery Box"),
			tiles = {
				top_tex,
				bottom_tex,
				side_tex,
			...
			on_construct = function(pos)
				...
				meta:set_string("infotext", S(""..tier.." Battery Box"))
