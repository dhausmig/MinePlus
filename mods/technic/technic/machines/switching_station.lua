...
minetest.register_abm({
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		...
		if meta:get_int("active") ~= 1 then
			...
			meta:set_string("infotext",S(""..machine_name.." Already Present"))
		...
		if tier then
			...
			meta:set_string("infotext", S(""..machine_name.." Has No Network"))
			minetest.forceload_free_block(pos)
			minetest.forceload_free_block(pos1)
			return
		end
...
--				machine_name, technic.pretty_num(PR_eu_supply), technic.pretty_num(RE_eu_demand)))
				S(""..machine_name..". Supply: "..technic.pretty_num(PR_eu_supply).." Demand: "..technic.pretty_num(RE_eu_demand)))
	...
	action = function(pos, node, active_object_count, active_object_count_wider)
		for tier, machines in pairs(technic.machines) do
			if machines[node.name] and switching_station_timeout_count(pos, tier) then
				...
				if nodedef then
					local meta = minetest.get_meta(pos)
					meta:set_string("infotext", S(""..nodedef.description.." Has No Network"))
				end
			end
		end
	end,
