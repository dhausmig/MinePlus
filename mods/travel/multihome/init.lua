...
local h_max  = tonumber( minetest.settings:get("multihome_max")) or 8
local compat = minetest.settings:get("multihome_compatibility") or "override"
local import = minetest.settings:get("multihome_import") or "false"
...
local function check_attr(player)
	if not player:get_meta():get("multihome") then
		player:get_meta():set_string("multihome", minetest.serialize({}))
	end
end
...
function multihome.set(player, name, pos)
	...
	local pos   = pos or vector.round(player:get_pos())
	local homes = minetest.deserialize(player:get_meta():get("multihome"))

	-- Check for space
	if count_homes(homes) >= h_max then
		return false, "Too many homes. Remove one with /delhome <name>"
	end

	homes[name] = pos
	player:get_meta():set_string("multihome", minetest.serialize(homes))
	...
	if homes then
		...
		for name, h in pairs(homes) do
			if list == "None" then
				...
				list = list..",\n"..name.." "..minetest.pos_to_string(h)
			end
		end
		return true, "Your Homes ("..count_homes(homes).."/"..h_max.."):\n"..list
	end
end
...
minetest.register_on_joinplayer(function(player)
	...
	if import == "true" and (compat == "deprecate" or compat == "override")
			and player:get_meta():get("multihome:imported") ~= "true" then
		local name = player:get_player_name()
		local pos = minetest.string_to_pos(player:get_meta():get("sethome:home")) or oldhomes[name]
		if pos then
			-- Set multihome entry
			multihome.set(player, "default", pos)
			-- Set imported attribute
			player:get_meta():set_string("multihome:imported", "true")
		end
	end
end)
...
if compat == "replace" then
	...
	minetest.register_privilege("home", {
		description = "Can use /home <name>, /sethome <name>, /delhome <name>, and /listhomes",
	...
	minetest.register_chatcommand("home", {
		...
		func = function(name, param)
			if param and param ~= "" then
				...
				return false, "Invalid parameters (usage: /home <name_of_home>)"
			end
		end,
	})
...
--				return false, "Invalid parameters (usage: /sethome <name_of_home>)"
				return multihome.set(name, "home")
			end
		end,
	})
	...
	minetest.register_chatcommand("delhome", {
		...
		func = function(name, param)
			if param and param ~= "" then
				...
				return false, "Invalid parameters (usage /delhome <name_of_home>)"
			end
		end,
	})
