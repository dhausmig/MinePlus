...
local function rollback_filling(player)
	...
	if player:get_meta():get("filler_deactivate") == "true" then
		minetest.chat_send_player(player_name, "Filling Tool: Deactivated.")
		player:get_meta():set_string("filler_deactivate", "false")
		player:get_meta():set_string("filler_activated", "false")
		return
	end
...
local function fill_area(cpos, bpos, epos, node, player, dpos, inv) --cpos, dpos and inv to improve performance
	...
	-- place node
	if not filler.endless_placeable[node.name] and not minetest.settings:get_bool("creative_mode") then
...
minetest.register_tool("filler:filler", {
	...
	on_use = function(itemstack, user, pointed_thing)
      ...
      return itemstack end
		...
		if user:get_meta():get("filler_activated") == "true" then
			...
			if user:get_player_control().sneak == true then
				user:get_meta():set_string("filler_deactivate", "true")
			end
			return
		end
		local meta = itemstack:get_meta()
		...
		user:get_meta():set_string("filler_activated", "true")
...
minetest.register_chatcommand("rsq", {
	...
	func = function(name, param)
		...
		if player:get_meta():get("filler_activated") == "true" then
			minetest.chat_send_player(name, "Filling Tool: The filling tool is currently working."..
				" (You can hold sneak and left click to deactivate it.)")
			return
		end
		player:get_meta():set_string("filler_activated", "true")
